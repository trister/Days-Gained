ggkm <- function(sfit,
  ...
  p <- ggplot( .df, aes(time, surv)) +
    geom_step(aes(linetype = strata), size = 1.5) +
    theme_grey(30) +
    opts(axis.title.x = theme_text(vjust = 0.5)) +
    scale_x_continuous(xlabs, breaks = times, limits = xlims) +
    scale_y_continuous(ylabs, limits = ylims) +
    opts(panel.grid.minor = theme_blank()) +
    opts(legend.position = c(ifelse(m < 10, .78, .35),ifelse(d < 4, .75, .35))) +    # MOVE LEGEND HERE [first is x dim, second is y dim]
  ...
  if(pval) {
    ...
    p <- p + annotate("text",x = 0.75 * max(sfit$time),y = 0.3,label = pvaltxt,size=12) 
  }
